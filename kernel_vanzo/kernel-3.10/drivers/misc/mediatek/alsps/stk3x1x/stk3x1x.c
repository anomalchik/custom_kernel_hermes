...
#ifdef FIH_ALS_CALI
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_alscali(struct device_driver *ddri, char *buf)
{
	if(!stk3x1x_obj)
	{
		APS_ERR("stk3x1x_obj is null!!\n");
	...
	return scnprintf(buf, PAGE_SIZE, "0x%04x(%d) 0x%04x(%d)\n", stk3x1x_obj->als_sample, stk3x1x_obj->als_sample, stk3x1x_obj->als_cali, stk3x1x_obj->als_cali);
}
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_alscali(struct device_driver *ddri, const char *buf, size_t count)
{
	int sample;
    int value;
    ...
    sscanf(buf, "0x%x 0x%x", &sample, &value);

	if(value <= 0 || sample <=0)
	{
		APS_ERR("%s: als calibration data not as expected: 0x%x 0x%x\n", __func__, sample, value);
	}
	else
	{
		stk3x1x_obj->als_sample = sample;
		stk3x1x_obj->als_cali = value;
	    APS_DBG("%s: als_sample = 0x%04x(%d), als_cali = 0x%04x(%d)\n", __func__, stk3x1x_obj->als_sample, stk3x1x_obj->als_sample, stk3x1x_obj->als_cali, stk3x1x_obj->als_cali);
	}
...
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(als,     S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_als,   NULL);
static DRIVER_ATTR(ps,      S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_ps,    NULL);
static DRIVER_ATTR(ir,      S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_ir,    NULL);
static DRIVER_ATTR(config,  S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_config,stk3x1x_store_config);
static DRIVER_ATTR(alslv,   S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_alslv, stk3x1x_store_alslv);
static DRIVER_ATTR(alsval,  S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_alsval,stk3x1x_store_alsval);
static DRIVER_ATTR(trace,   S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_trace, stk3x1x_store_trace);
static DRIVER_ATTR(status,  S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_status,  NULL);
static DRIVER_ATTR(send,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_send,  stk3x1x_store_send);
static DRIVER_ATTR(recv,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_recv,  stk3x1x_store_recv);
static DRIVER_ATTR(reg,     S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_reg,   NULL);
static DRIVER_ATTR(allreg,  S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_allreg,   NULL);

#ifdef STK_TUNE0
static DRIVER_ATTR(cali,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_cali,  NULL);
static DRIVER_ATTR(maxdiff,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_ps_maxdiff_show,  stk3x1x_ps_maxdiff_store);
static DRIVER_ATTR(ltnct,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_ps_ltnct_show,  stk3x1x_ps_ltnct_store);
static DRIVER_ATTR(htnct,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_ps_htnct_show,  stk3x1x_ps_htnct_store);
#endif
#ifdef STK_ALS_FIR
static DRIVER_ATTR(firlen,    S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_firlen,  stk3x1x_store_firlen);
#endif

#ifdef FIH_ALS_CALI
static DRIVER_ATTR(alscali,  S_IWUSR | S_IRUGO | S_IWGRP, stk3x1x_show_alscali,stk3x1x_store_alscali);
#endif
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
    ...
    if( !obj->als_cali )
    {
	    APS_DBG("ALS: als_cali = 0!!!\n" );
	    return	0;
    }

    return (als * obj->als_sample) / obj->als_cali;
...
static int32_t stk3x1x_set_irs_it_slp(struct stk3x1x_priv *obj, uint16_t *slp_time, int32_t ials_it_reduce)
{
	...
	//APS_LOG("(smbus) timing:%d\n", obj->client->timing);
	//ret = i2c_smbus_write_byte_data(obj->client, STK_ALSCTRL_REG, irs_alsctrl);
	ret = stk3x1x_master_send(obj->client, STK_ALSCTRL_REG, &irs_alsctrl, 1);
	if (ret < 0)
	{
...
static int32_t stk3x1x_get_ir_value(struct stk3x1x_priv *obj, int32_t als_it_reduce)
{
	...
	ret = stk3x1x_master_send(obj->client, STK_STATE_REG, &w_reg, 1);
    if (ret < 0)
	{
		APS_ERR( "%s: write i2c error\n", __func__);
		goto irs_err_i2c_rw;
	...
	alsctrl = atomic_read(&obj->alsctrl_val);
	ret = stk3x1x_master_send(obj->client, STK_ALSCTRL_REG, &alsctrl, 1);
	if (ret < 0)
	{
		APS_ERR( "%s: write i2c error\n", __func__);
		goto irs_err_i2c_rw;
