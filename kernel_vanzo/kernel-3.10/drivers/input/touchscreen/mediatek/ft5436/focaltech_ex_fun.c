...
//	#include "FT5436I_ID5f_J6688_V0x11_20150813_app.i"
//};  
static unsigned char CTPM_FW1[] = {
	//#include "FT5436I_J6688_ID5F_V0x12_20150827_app.i"
	//#include "FT5436I_J6688_ID5F_V0x14_20150910_app.i"
};  
static unsigned char CTPM_FW2[] = {
	//#include "FT5436I_ID67_J6688_V0x11_20150827_app.i"
	//#include "FT5436I_ID67_J6688_V0x12_20150901_app.i"
};  
	...
	*/
	...
	//	&& (CTPM_FW[fw_len - 3] ^ CTPM_FW[fw_len - 4]) == 0xFF) {
		...
		/*call the upgrade function */
			i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW1));
		}
	else if(vid_xinan==0x67)
		{
			pbt_buf = CTPM_FW2;
		/*call the upgrade function */
			i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW2));
		}
	//	pbt_buf = CTPM_FW;
		/*call the upgrade function */
...
unsigned char fts_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
		printk("xinan entry fts_ctpm_VidFWid_get_from_boot()\n");
...
{
	printk("xinan entry fts_ctpm_auto_upgrade()\n");
	...
	fts_read_reg(client, FT_REG_VENDOR_ID, &uc_tp_vendor_id);
	//vid_xinan = fts_ctpm_VidFWid_get_from_boot( client);
	vid_xinan=uc_tp_vendor_id;
  printk("xinan uc_tp_fm_ver = 0x%x, uc_tp_vendor_id = 0x%x\n,xinan VID= 0x%x",uc_tp_fm_ver, uc_tp_vendor_id,vid_xinan);
	if(0)
	{
	if((0x5f != uc_tp_vendor_id) && (0x67 != uc_tp_vendor_id))
	{
	  printk("[FST] reg_module_id do not known error !\n");
	  //vid = fts_ctpm_VidFWid_get_from_boot( client);
    printk("xinan VID= 0x%x!\n", vid_xinan);
	  if ( vid_xinan == 0xFF )
	  ...
	  else
	  {
       ...
       printk("xinan uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n,xinan VID= 0x%x",uc_tp_fm_ver, uc_host_fm_ver,vid_xinan);
